                  ...
                  >
                    ...
                    {message.role === "assistant" && (
                      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/10">
                        ...
                        <button
                          onClick={() => {
                            if (feedbackMap[message.id] === "good") {
                              setFeedbackMap(prev => ({ ...prev, [message.id]: null }));
                            } else {
                              toast.success("We got your feedback");
                              setFeedbackMap(prev => ({ ...prev, [message.id]: "good" }));
                            }
                          }}
                          className="transition-colors"
                          title="Good response"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className={`h-3.5 w-3.5 ${feedbackMap[message.id] === "good" ? "text-green-400 fill-green-400" : "text-gray-400 hover:text-green-400"}`} viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
                          </svg>
                        </button>
                        <button
                          onClick={() => {
                            if (feedbackMap[message.id] === "bad") {
                              setFeedbackMap(prev => ({ ...prev, [message.id]: null }));
                            } else {
                              toast.success("We got your feedback");
                              setFeedbackMap(prev => ({ ...prev, [message.id]: "bad" }));
                            }
                          }}
                          className="transition-colors"
                          title="Bad response"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className={`h-3.5 w-3.5 ${feedbackMap[message.id] === "bad" ? "text-red-400 fill-red-400" : "text-gray-400 hover:text-red-400"}`} viewBox="0 0 24 24" stroke="currentColor">
